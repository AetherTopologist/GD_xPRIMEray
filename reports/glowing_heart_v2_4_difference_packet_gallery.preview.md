...
| NotComparable Incompatible Channel | base bend vs base traversal steps | bend vs steps | `NotComparable` | `traversal_steps_vs_bend_not_comparable` | 0 | 0 | 0 | 0 | Declared incompatible retained Core quantities are rejected before value comparison. |
...
- `Deferred`: the correct retained artifact is not available; no current entry uses this status after v2.5.
...
Every represented Difference Packet preserves `runtimeExecuted=false`, `parityClaim=NONE`, and `comparisonMode=core_vs_core`. Case E now has an authentic packet; bend data was not relabeled.
